...
This mechanism enables the model to capture long-range dependencies more effectively than RNNs or CNNs, leading to better performance on sequence modeling tasks.""",
        ...
        def query(self, question: str):
            ...
            class RAGResponse:
                def __init__(self, answer, sources):
                    ...
                    self.confidence_score = 0.87 if sources else 0.65
            ...
            for keywords, doc_info in self.knowledge_base.items():
                ...
                if relevance_score > 0.1:
                    scored_docs.append({
                        ...
                        'page': doc_info["page"],
                        'score': min(relevance_score, 0.95),
        ...
        def _generate_with_openai(self, question, docs):
            ...
            try:
                st.info(f"🔑 OpenAI API Key Status: {'Found' if self.has_openai else 'Missing'}")

                if not self.has_openai:
                    st.warning("⚠️ No OpenAI API key detected - using fallback")
